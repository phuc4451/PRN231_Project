...
namespace Project_PRN231_Client.Controllers
{
    public class LoginController : Controller
    {
        ...
        public async Task<IActionResult> Index(User user)
        {
            try
            {
                HttpResponseMessage response = await _client.PostAsJsonAsync(_apiBaseUrl + "User/Login?$expand=Role,CareProcesses,HarvestProcesses,PlantProcesses", user);
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        HttpContext.Session.SetString("role", JsonConvert.SerializeObject(loggedInUser.Role.RoleName));
                    ...
                    else if (string.Equals(loggedInUser.Role.RoleName, "manager", StringComparison.OrdinalIgnoreCase))
                    {
                        HttpContext.Session.SetString("role", JsonConvert.SerializeObject(loggedInUser.Role.RoleName));
                        ViewBag.LoginSuccess = true;
                        return Redirect("/Home");
